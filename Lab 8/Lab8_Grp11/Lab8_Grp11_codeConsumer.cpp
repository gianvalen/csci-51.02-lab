...
int main(int argc, char* argv[]) {
    ...
    // No IPC_CREAT because it is not responsible for creating the semaphore
    ...
    int data_shmid = shmget(DATA_KEY, shm_size, 0666);
    int status_shmid = shmget(STATUS_KEY, 2, 0666);

    // Check if shared memory creation was successful
    if (data_shmid < 0 || status_shmid < 0) {
        perror("shmget");
        return 1;
    }
    ...
    while (1) {
        ...
        // Check if data is written and ready to be read
        if (status[0] == 'W') {
            // Get the number of valid bytes from shared memory
            int valid_bytes = *((int*)shmat(shmget(SIZE_KEY, sizeof(int), 0666), NULL, 0));
            
            // Only write the valid bytes, not the padded zero bytes
            fwrite(data, 1, valid_bytes, outfile);
            status[0] = 'R'; // Update the status to 'R' to indicate data has been read
        
        // If status is 'D', it means the producer is done writing
        } else if (status[0] == 'D') {
